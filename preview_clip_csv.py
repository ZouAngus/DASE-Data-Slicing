...
csv_folder    = "/home/angus/cv_slice_data/output/csv_01_boss_C"
video_folder  = "/home/angus/cv_slice_data/output/Clips_01_boss_C.mp4"
...
# build a map of (row,rep) → video file
video_map = {}
for vid in sorted(os.listdir(video_folder)):
    if not vid.lower().endswith(".mp4"):
        continue
    m = re.match(r"row(\d+)_rep(\d+)_frames.*\.mp4$", vid)
    if m:
...
for csv_name in sorted(os.listdir(csv_folder)):
    ...
    # parse row & rep from CSV name, e.g. csv_01_travel_C_23_rep6.csv
    m = re.match(r".+_([0-9]+)_rep([0-9]+)\.csv$", csv_name)
    if not m:
        print(f"[WARN] skipping unrecognized csv name: {csv_name}")
        continue
    row_i, rep = m.group(1), m.group(2)
    csv_path = os.path.join(csv_folder, csv_name)

    # lookup matching video
    video_path = video_map.get((row_i, rep))
    if not video_path:
        print(f"[WARN] no clip for row{row_i}_rep{rep}, skipping")
        continue

    base = f"row{row_i}_rep{rep}"
    out_path = os.path.join(output_folder, f"{base}_preview.mp4")
